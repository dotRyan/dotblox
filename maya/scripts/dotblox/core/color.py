...
def color_managed_convert(rgbf):
    ...
    cm_ocio_enabled = cmds.colorManagementPrefs(query=True, cmConfigFileEnabled=True)
    if cm_ocio_enabled:
        cmds.warning("OCIO config enabled. 2.2 Gamma is being used for color conversion")
